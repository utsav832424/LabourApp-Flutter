...
class Staff_NameDetail extends StatefulWidget {
  Staff_NameDetail(
      {Key? key,
  ...
  @override
  State<Staff_NameDetail> createState() => _Staff_NameDetailState();
}

class _Staff_NameDetailState extends State<Staff_NameDetail> {
  ...
  init() async {
    ...
    var resData = await apiService.getcall(
        "staff/getnameByidC?from_date=${txtController.text}&to_date=${txtController2.text}&user_id=${widget.user_id}&name=${widget.name}");
    log("${resData}");
    if (mounted) {
      setState(() {
        data = resData['data'];
