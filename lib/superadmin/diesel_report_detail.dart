...
import 'package:abc_2_1/superadmin/dieselreportvehicalno.dart';
...
class SDiesel_report_detail extends StatefulWidget {
  SDiesel_report_detail(
      {Key? key, required this.machinery, required this.refresh})
      : super(key: key);
  final machinery;
  final void Function(Map<String, dynamic> filters) refresh;

  @override
  State<SDiesel_report_detail> createState() => _SDiesel_report_detailState();
}

class _SDiesel_report_detailState extends State<SDiesel_report_detail> {
  ...
  init() async {
    ...
    var resData = await apiService.getcall(
        "diesel?from_date=${txtController.text}&to_date=${txtController2.text}&offset=0&length=20&user_id=${widget.machinery}");
    log("${resData}");
    if (mounted) {
      setState(() {
        diesel = resData['data'];
