...
class MaterialRecieve_Supplier extends StatefulWidget {
  const MaterialRecieve_Supplier(
      {super.key, required this.supplierNameM, required this.adminid});
  final supplierNameM;
  final adminid;
  @override
  State<MaterialRecieve_Supplier> createState() =>
      _MaterialRecieve_SupplierState();
}

class _MaterialRecieve_SupplierState extends State<MaterialRecieve_Supplier> {
  ...
  init() async {
    ...
    var resData = await apiService.getcall(
        "materials/getSupplier?from_date=${txtController.text}&to_date=${txtController2.text}&offset=0&length=20&user_id=${widget.adminid}&suppliername=${widget.supplierNameM}");
    log("${resData}");
    if (mounted) {
      setState(() {
