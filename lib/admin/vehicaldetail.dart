...
class Vehicalno extends StatefulWidget {
  Vehicalno({Key? key, required this.vehicalno}) : super(key: key);
  final vehicalno;

  @override
  State<Vehicalno> createState() => _VehicalnoState();
}

class _VehicalnoState extends State<Vehicalno> {
  ...
  init() async {
    ...
    var resData = await apiService.getcall(
        "materials/getVehical?from_date=${txtController.text}&to_date=${txtController2.text}&offset=0&length=20&user_id=${userid}&vehical=${widget.vehicalno}");
    log("${resData}");
    if (mounted) {
      setState(() {
