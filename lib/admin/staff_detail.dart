...
class Staff_Detail extends StatefulWidget {
  const Staff_Detail({super.key, required this.name});
  final name;
  @override
  State<Staff_Detail> createState() => _Staff_DetailState();
}

class _Staff_DetailState extends State<Staff_Detail> {
  ...
  init() async {
    ...
    var resData = await apiService.getcall(
        "staff/getnameByidC?from_date=${txtController.text}&to_date=${txtController2.text}&user_id=${userid}&name=${widget.name}");
    log("${resData}");
    if (mounted) {
      setState(() {
        data = resData['data'];
